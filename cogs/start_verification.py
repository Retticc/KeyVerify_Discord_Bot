...
class StartVerification(commands.Cog):
    ...
    async def start_verification(self, inter: disnake.ApplicationCommandInteraction):
        ...
        if not products:
            await inter.response.send_message("❌ No products are registered for this server.", ephemeral=True,delete_after=message_timeout)
            return
        ...
        async with (await get_database_pool()).acquire() as conn:
            ...
            if result:
                try:
                    ...
                    await inter.response.send_message("✅ Verification message updated successfully.", ephemeral=True,delete_after=message_timeout)
                    ...
                    await inter.response.send_message("✅ New verification message created successfully.", ephemeral=True,delete_after=message_timeout)
            else:
                new_message = await inter.channel.send(embed=embed, view=view)
                await conn.execute(
                    """
                ...
                await inter.response.send_message("✅ Verification message created successfully.", ephemeral=True,delete_after=message_timeout)
